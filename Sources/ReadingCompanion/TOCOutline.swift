...
enum TOCInputNormalizer {
    ...
    static func cleanEntryTitle(_ source: String) -> String {
...
enum TOCTextParser {
    static func parse(_ source: String) -> [TOCSourceEntry] {
        let cleanedLines = TOCInputNormalizer.normalize(source)
            .components(separatedBy: .newlines)
            .map(cleanLine)
            .flatMap(splitCombinedEntries)
            .filter { !$0.isEmpty && !isNoise($0) }
        let lines = pairDetachedPageColumns(in: cleanedLines)
    ...
    private static func pairDetachedPageColumns(in lines: [String]) -> [String] {
        ...
        while index < result.count {
            ...
            while titleStart > 0, ordinaryTitleCount < pageCount {
                ...
                if !isUnpagedDivisionTitle(candidate) { ordinaryTitleCount += 1 }
            }
            ...
            let paired = titles.map { title -> String in
                guard !isUnpagedDivisionTitle(title) else { return title }
                defer { pageIndex += 1 }
    ...
    private static func isUnpagedDivisionTitle(_ title: String) -> Bool {
        isPart(TOCInputNormalizer.cleanEntryTitle(title)) && titleAndPage(title) == nil
    }
    ...
    private static func inheritingPagesForDivisions(
        ...
        for index in result.indices.reversed() {
            ...
            } else if isUnpagedDivisionTitle(result[index].title), let nextPage {
    ...
    private static func cleanTitle(_ source: String) -> String {
        TOCInputNormalizer.cleanEntryTitle(source)
    }
...
enum ManualTOCTextParser {
    ...
    static func parse(_ source: String) -> [TOCSourceEntry] {
        let candidates = TOCInputNormalizer.normalize(source)
            .components(separatedBy: .newlines)
            .enumerated()
            .compactMap(makeCandidate)
        guard !candidates.isEmpty else { return [] }

        let sequencePosition = preferredSequencePosition(in: candidates)
        let parsed = candidates.compactMap { parse($0, sequencePosition: sequencePosition) }
        return TOCHierarchyInferer.apply(to: reorderNumberedEntries(parsed).map(\.entry))
    }

    private static func makeCandidate(index: Int, line: String) -> Candidate? {
        let halfWidth = line.applyingTransform(.fullwidthToHalfwidth, reverse: false) ?? line
    ...
    private static func parse(_ candidate: Candidate, sequencePosition: Int) -> ParsedLine? {
        ...
        if let explicitSequence {
            titleTokens.insert(cleanSequenceToken(explicitSequence), at: 0)
        }
        let title = TOCInputNormalizer.cleanEntryTitle(titleTokens.joined(separator: " "))
        guard title.count >= 2, title.count <= 180 else { return nil }
...
enum TOCReliableParser {
    static func parse(_ source: String, preferLinewise: Bool = false) -> [TOCSourceEntry] {
        let linewise = ManualTOCTextParser.parse(source)
        let flexible = TOCTextParser.parse(source)
        ...
        for segment in Set(segmentIDs).sorted() {
            ...
            for index in entries.indices where segmentIDs[index] == segment {
                ...
                let selected = pool.max { left, right in
                    anchorScore(left, entryIndex: index, previousEntry: previousEntry, previousPage: previousPage, entries: entries)
                        < anchorScore(right, entryIndex: index, previousEntry: previousEntry, previousPage: previousPage, entries: entries)
                }!
