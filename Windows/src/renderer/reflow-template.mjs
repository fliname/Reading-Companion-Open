// Generated by scripts/sync-mac-reflow.cjs from macOS 0.55.6. Do not hand edit.
export const TEMPLATE = [
  ...
  "  }",
  "  document.addEventListener('mouseup',()=>setTimeout(()=>{const payload=selectionPayload();if(payload)window.parent.postMessage({type:'reader:selection',...payload},'*');},0));",
  ...
  "      if(hit){event.preventDefault();event.stopPropagation();window.parent.postMessage({type:'reader:mark',id:hit.id,x:hit.left,y:hit.top,width:hit.width,height:hit.height},'*');return;}",
  "    }",
  ...
  "  window.readerConfigure=(scale,count,saved,location,legacyPage)=>{document.documentElement.style.setProperty('--reader-scale',String(scale));requestedSpread=count===2?2:1;layout(false,false);if(saved)restorePosition(saved);else{currentPage=Number.isInteger(legacyPage)?legacyPage:pageForLocation(location||0);position(false);renderMarks();}initialized=true;report();};",
  ...
  "  function report(){window.parent.postMessage({type:'reader:navigation',location:locationAtPage(currentPage),pageNumber:currentPage+1,pageCount,spreadCount:spread,initialized,layoutRevision,position:readingPosition()},'*');}",
