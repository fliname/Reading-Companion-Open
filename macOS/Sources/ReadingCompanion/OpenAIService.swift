...
actor OpenAIService {
    ...
    func generateOutlineFromTOC(
        ...
        let localCandidates = TOCReliableParser.parse(contents, legacyAutomatic: true)
        ...
        let rawAIEntries = payload.entries.compactMap { entry -> TOCSourceEntry? in
            let title = TOCInputNormalizer.cleanEntryTitle(entry.title, legacyAutomatic: true)
        ...
        let aiEntries = rawAIEntries.flatMap { entry -> [TOCSourceEntry] in
            ...
            let separated = TOCTextParser.parse(entry.title + pageSuffix, legacyAutomatic: true)
        ...
        let entries = TOCPageResolver.resolve(
            sourceEntries,
    ...
    static func responseBudgetInstructions(for depth: AIReadingDepth) -> String {
        let target: String
        switch depth {
        case .economical: target = "约 448–640 个汉字，最多 3 个短节"
        case .balanced: target = "约 768–1,152 个汉字，最多 4 个短节"
        case .deep: target = "约 1,280–1,920 个汉字，最多 6 个短节"
        }
        return """
