...
enum HighlightFragmentNormalizer {
    static func normalize(_ fragments: [HighlightFragment]) -> [HighlightFragment] {
        var merged: [HighlightFragment] = []
        let ordered = fragments.filter { !$0.bounds.cgRect.isEmpty }.sorted(by: readingOrder)
        for fragment in ordered {
            let rect = fragment.bounds.cgRect.standardized
            guard let index = merged.firstIndex(where: {
                $0.pageIndex == fragment.pageIndex && belongsToSameVisualLine($0.bounds.cgRect, rect)
            }) else {
    ...
    private static func readingOrder(_ lhs: HighlightFragment, _ rhs: HighlightFragment) -> Bool {
        if lhs.pageIndex != rhs.pageIndex { return lhs.pageIndex < rhs.pageIndex }
        let left = lhs.bounds.cgRect
        let right = rhs.bounds.cgRect
        if abs(left.midY - right.midY) > max(min(left.height, right.height) * 0.45, 2) {
...
struct HighlightRecord: Identifiable, Codable, Hashable {
    ...
    var allFragments: [HighlightFragment] {
        ...
        return HighlightFragmentNormalizer.normalize(fragments)
    }

    var inlineText: String { HighlightTextNormalizer.inline(text) }
    var isMergedList: Bool {
        let lines = text.components(separatedBy: .newlines)
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        return lines.count >= 2 && lines.allSatisfy { $0.hasPrefix("•") }
    }
    /// Ordinary PDF line wraps remain one readable sentence. Records created
    /// by the explicit merge command retain one bullet per source mark.
    var displayText: String {
        guard isMergedList else { return inlineText }
        return text.components(separatedBy: .newlines).compactMap { rawLine in
            ...
            return "• \(HighlightTextNormalizer.inline(String(body)))"
        }.joined(separator: "\n")
    }
