...
当前版本：0.55.6（Build 114）。这是可与本地版 Reading Companion 并存的公开版，使用独立的应用名称、Bundle ID、设置、缓存和密钥目录。

一个原生 macOS PDF / EPUB / AZW3 / MOBI 伴读应用：电子书流式排版、本地 OCR、带位置 AI 问答、三色划线与 Obsidian 结构化笔记。

0.55.6 更新：电子书图片缓存强制重建，并按阅读页可用高度约束大图，避免 EPUB / AZW3 / MOBI 图片在分栏排版中消失；页面下方选区的划线浮窗固定显示在选区下方。此前的 Obsidian、全局方向键、书架文件夹删除、已有划线编辑和 `ljg-read` 开关修改继续保留。详见 [本版说明](MAC_RELEASE_0.55.6.md)。

0.54.0 更新：修复虚构类概要切换时重新分配三栏宽度、导致阅读页面被挤窄的问题；概要现在在目录栏原位稳定覆盖。页码范围不再近似换算到电子书内部固定分页，而是直接按当前流式阅读器每一页的实际排版提取文字，修复概要与输入页码不对应的问题。非虚构类概要流程不变。

## 第一次使用：从安装到生成笔记

### 1. 安装公开版

1. 打开 `Reading-Companion-Open-0.55.6-macOS-arm64.dmg`。
...
4. 导入书籍时选择“非虚构类”或“虚构类”。类型会固定到这本书，之后显示在“AI 伴读”标题右侧；非虚构类还可选择阅读深度。
...
### 4. 导入 PDF / EPUB / AZW3 / MOBI 并识别目录

1. 点击“打开”或把 PDF、EPUB、AZW3、MOBI 书籍文件拖入应用。
2. PDF 文本页会直接提取，扫描件会调用 macOS Vision 在本机 OCR；其他电子书会解析为可随字号和阅读区宽度实时换行的流式正文，再建立全文索引。
3. 打开左侧“目录”。应用先检查文件自带目录；没有可信目录时可定位正文目录页或使用“手动添加”。
...
2. 点击“添加目录”，把当前已经校对的目录作为章节骨架写入该笔记。
...
- 原生三栏 PDF / EPUB / AZW3 / MOBI 阅读界面，四种格式均支持打开和拖放。
- EPUB 3 `nav` 与 EPUB 2 `NCX` 目录会在导入时提取；AZW3/MOBI 先由随包提供的 libmobi 在本机离线解析，再进入同一电子书管线。导入、排版和索引阶段均显示独立进度与电子书专用文案。正文采用白底、系统中文无衬线字体、舒展行距和层级化章节标题；调整字号或阅读区宽度时实时重排，并始终显示动态页界和页码。滚轮或触控板达到阈值后立即翻页，动画约 150ms；顶栏可选自动、单页或双页，自动模式在左右侧栏同时收起时切换双页。结果会缓存，源文件修改或渲染版本升级后自动重建。带 DRM 的 Kindle 文件不受支持。
- 启动后自动检查 GitHub Release；发现对应 macOS 新安装包时可选择立即下载并打开、稍后提醒或忽略该版本。
...
- 页码跳转、方向键翻页、工具栏与书签侧栏的明确入口（⌘D 添加当前页）、全文搜索。提交搜索只生成结果与高亮并保留当前页；点击左侧具体搜索条目后，精确跳转到该次命中的文字，同一页出现多个相同词语时也按所点击的条目分别定位。
- PDF 与电子书共用同一套选区浮窗，把“划线”“批注”“复制”“提问”分开：三色色块直接完成划线；绿色下划线在原文下方打开无边框两行批注框，回车发送、Shift+回车换行、点击框外取消；提问恢复系统气泡图标。电子书划线按逐文字片段绘制，不覆盖行距或页面两侧空白。工具栏划线图标可切换持续划线模式。
...
- 非虚构类在生成前按阅读深度规划：节省为 1,500 Token、均衡为 3,000 Token、深读为 5,000 Token。快捷问题区的 `ljg-read` 开关默认开启；关闭后使用基础原文问答，并按书保存选择。虚构类面向小说等叙事阅读，不显示快捷问题，不加载学术框架；事实问答或适度文学分析默认控制在 120–300 个汉字，同时保留“联系全书”。
...
- 目录右侧“概要”按书籍类型工作：非虚构类保持按目录章节展开“问题—论证—结论—承上/启下”结构卡；虚构类则用概要栏覆盖目录栏，按输入的阅读器起止页码生成 100–220 字自然段。虚构类每次生成的页码范围与概要以独立卡片持久化保存，可滚动浏览；两类概要均只保存在应用内。
- 书架采用更大的封面网格，可新建文件夹并通过右键菜单整理书籍；书名前的蓝色/粉色圆点分别表示非虚构类/虚构类，整理只改变书架分组，不移动原文件。
- 虚构类书籍额外显示“人物”导航；点击人物索引右上角“管理”后，右侧人物管理栏才暂时代替 AI 对话。支持按“人物：身份，关系等信息”逐行批量识别，也可逐个添加；已添加人物可直接修改或删除。每个人物自动获得不重复的独立颜色，其全部别名在全文同色高亮，第一次出现额外加粗。人物展开后最多列出以当前页最近位置为中心的 11 次出现，边界处自动向另一侧补足，并支持上一次/下一次精确跳转。
